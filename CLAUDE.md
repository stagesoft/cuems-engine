...
- **Node adoption from the UI (`nodelist_modify`) is a three-hop chain, and the engine hop is new.** browser → editor (WS `:9092`) → engine (`/tmp/editor.ipc`) → nodeconf (`/tmp/nodeconf.ipc`). The engine end lives in `ControllerEngine.nodelist_modify` and is **deliberately not in `command_dict`**: every entry there is invoked as `(value, context)`, a two-argument contract with nowhere to put `modify_action`, so a table-registered handler would send `modify_action=None` and nodeconf would answer `Invalid modify_action: None` to *every* click. The message is bound before the table and dispatched directly. Three more things that bite:
  - **A missing nodeconf is the common case** (it ships disabled on most of the fleet), and it is *not* cheap to discover: measured on the rig, with `/tmp/nodeconf.ipc` absent `request_to_nodeconf` does **not** return quickly — it burns the full 15 s IPC timeout and then raises `TimeoutError`. Editor commands are serialized by the engine's single listener, so a second click queues behind the first and can blow the editor's own 25 s timeout, turning a clear refusal into *"Engine did not respond"*. Hence the `os.path.exists(NODECONF_IPC_PATH)` pre-check (instant, actionable refusal — measured 0.00 s) and the explicit 5 s `timeout=` on the call. The `None` return is still handled: `Communicator.send_request` swallows exceptions and returns it on other failures.
...
- **A project node that is missing or silent WARNS — it never blocks GO.** `_resolve_cluster_state` computes `required = (adopted & alive & project) | {controller}`, so a project node that is not adopted, or adopted but did not pong, is simply **excluded** from the arm gate: the load succeeds, GO is enabled, and that node's cues never fire. That is a deliberate operator decision (a mute cue beats a dead show), and the diagnosis is published rather than left in the journal:
  - `missing` = project nodes not in the map at all; `unreachable` = adopted, used, did not answer. The controller's own UUID is excluded from both — it is in `required` unconditionally, and it is not always flagged `adopted` in `network_map.xml`, so including it would raise a false alarm on a healthy load.
  - Three surfaces, all carrying the same two lists: pushed at load as `/engine/status/cluster_warning` (a **JSON string**, since OSC cannot carry lists), replayed to every new WebSocket client in `_on_ws_client_connect` (with boot auto-load the browser is normally opened *after* the load), and readable on demand in `get_cluster_status` → the editor's `node_status`.
  - **Always sent, empty lists included** — a clean load has to actively erase the previous warning. Cleared on `unload_project` and at the top of `load_project`, so a load that fails early does not leave the previous project's warning on screen. Not cleared by STOP: a stopped project is still loaded, and its missing nodes are still missing.
  - `load_id` increments per load so a UI can tell an operator's retry (new id, identical lists) from a reconnect replay (same id).
  - ⚠ It is only as good as the project's output mapping: `_collect_project_nodes` derives node UUIDs from `output_name[:36]`, so a project whose outputs are unmapped or malformed yields an empty `project` set and the alert says nothing is wrong.
