...
for data in ossia.ossia.list_oscquery_devices():
...
int_parameter.repetition_filter = ossia.ossia.RepetitionFilter.Off
...
string_node.instance_bounds = ossia.ossia.InstanceBounds(1, 10)
...
device_callback = ossia.ossia.DeviceCallback(remote_oscquery_device, on_creation_callback, on_renamed_callback, on_removing_callback)
...
remote_osc_device = ossia.ossia.OSCDevice("remoteOSCDevice", "127.0.0.1", 10000, 10001)
...
midi_devices = ossia.ossia.list_midi_devices()
...
if len(midi_devices) > 0:
  ...
  node_vector = ossia.ossia.list_node_pattern([remote_midi_device.root_node], "/1/control/*")
