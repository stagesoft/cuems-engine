...
1. **Body** built by `ActionHandler._build_fade_payload(target_cue, fade_cue, start_time, fade_id)`
   — returns `list[dict]`. AudioCue produces a single-entry list; VideoCue
   produces one entry per `_layer_ids` member, each with its own `osc_path` and
   layer-suffixed `fade_id`. Each dict carries `fade_id` so the handler knows
   which to pass to `send_fade_command` when iterating.
2. **Envelope** injected by `NodeCommunications.send_fade_command(payload, fade_id)` —
   `command`, `osc_host`, `curve_params` (and `fade_id` from the kwarg).

| Field | Source | Layer | Notes |
|-------|--------|-------|-------|
| `command` | Literal `"start_fade"` | envelope | Discriminator for gradient-motiond |
| `fade_id` | `str(FadeCue.uuid)` (audio) or `f"{uuid}_{layer_id}"` (video) | envelope (via kwarg) | Controller-assigned correlation key. AudioCue: base. VideoCue: one per layer. |
...
| `osc_path` | `"/volmaster"` (audio) or `"/videocomposer/layer/{layer_id}/opacity"` (video) | body | From target_cue. One per `_layer_ids` member for VideoCue. |
| `start_value` | `target_cue._osc.get_value(osc_path)` | body | Float, current cached OSC value (per layer for video) |
| `target_value` | `FadeCue.target_value` | body | Integer 0–100 (UI scale, NOT normalised); shared across video layers |
| `start_time` | `CTimecode.milliseconds_rounded` (int) | body | MTC clock at dispatch time; shared across video layers |
| `duration_ms` | `FadeCue.duration.milliseconds_rounded` (int) | body | Must be > 0; shared across video layers |
| `curve_type` | `FadeCue.curve_type.value` (str) | body | `"linear"` / `"exponential"` / `"logarithmic"` / `"sigmoid"`; shared across video layers |
...
| target_cue type | dispatch count | osc_host | osc_port | osc_path |
|-----------------|----------------|----------|----------|----------|
| `AudioCue` | 1 | `"127.0.0.1"` | `target_cue._osc.remote_port` | `"/volmaster"` |
| `VideoCue` | `len(target_cue._layer_ids)` | `"127.0.0.1"` | `7000` | `f"/videocomposer/layer/{layer_id}/opacity"` for each `layer_id` |
