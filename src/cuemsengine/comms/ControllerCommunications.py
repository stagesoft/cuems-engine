...
class ControllerCommunications(AsyncCommsThread):
    ...
    def register_osc_handler(
        self, osc_pattern: str, handler: Callable[[str, list], None]
    ) -> None:
    ...
    def reply_to_editor(self, message, context: Context):
        send_task = asyncio.run_coroutine_threadsafe(
            self.editor.responder_post_reply(message, context), self.event_loop
        )
        try:
