...
class BaseEngine(SignalEngine):
    ...
    def read_script(self, project_name: str) -> None:
        xml_file = path.join(
            self.cm.library_path, "projects", project_name, "script.xml"
        )
    ...
    def initial_cuelist_process(self, cuelist: CueList = None):
        ...
        if (
            not hasattr(cuelist, "contents")
            or not cuelist.contents
            or len(cuelist.contents) == 0
        ):
