...
def arm_videoCue(cue: VideoCue):
    ...
    for index, output_name in enumerate(output_names):
        ...
        try:
            ...
            Logger.warning(
                f'Video output "{output_name}" placement/scale failed ({type(e).__name__}: {e}), skipping for layer {layer_id}'
            )
        except Exception:
            Logger.exception(
                f'Unexpected error setting placement/scale for layer {layer_id} (output "{output_name}")'
            )
