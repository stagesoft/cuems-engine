...
def mock_config_manager():
    with patch("cuemsutils.tools.ConfigManager.ConfigManager") as mock_cm:
        mock_cm.node_conf = {"uuid": "test_node", "mtc_port": MTC_PORT}
...
def mock_avahi_resolve():
    ...
    with patch(
        "cuemsengine.tools.CuemsDeploy.CuemsDeploy._avahi_resolve",
...
def mock_player_clients():
    ...
    client_records = {"clients": [], "commands": []}
    ...
    with (
        patch("cuemsengine.players.VideoPlayer.VideoClient", MockVideoClient),
...
def mock_library_path(monkeypatch):
    ...
    monkeypatch.setattr(
        "cuemsutils.tools.ConfigManager.ConfigManager.library_path",
