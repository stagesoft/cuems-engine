...
def _cue(id, local, enabled, eff, post_go='go', target=None):
    ...
    return SimpleNamespace(
        id=id, _local=local, enabled=enabled, _eff=eff,
        post_go=post_go, _target_object=target,
    )
...
def _go_threaded_cue(prewait_ms=1000, postwait_ms=0, post_go='pause',
                     go_gen=7, cur_gen=7, target=None):
    return SimpleNamespace(
        id='c', _local=True, _stop_requested=False,
        _go_generation=cur_gen, post_go=post_go, _target_object=target,
...
class TestGoThreadedAnchoring:
    ...
    def test_start_ms_is_arrival_plus_prewait(self):
        ...
        mtc = _mtc()
        with patch('cuemsengine.cues.CueHandler.run_cue') as run_cue, \
    ...
    def test_fires_next_at_returned_arrival(self):
        ...
        mtc = _mtc()
        with patch('cuemsengine.cues.CueHandler.run_cue'), \
    ...
    def test_superseded_generation_does_not_fire(self):
        ...
        cue, go_gen = _go_threaded_cue(post_go='go', cur_gen=8, go_gen=7,
