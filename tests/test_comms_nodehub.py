...
def test_node_operation_serialization_format():
    ...
    assert str_repr == f"NodeOperation by {sender_id}: add on player {player_id} (with data)"
    ...
    assert str(remove_op) == f"NodeOperation by {sender_id}: remove on player {player_id} (without data)"
...
class TestNodesHubIntegration:
    ...
    def test_send_operation_from_node_to_controller(self):
        ...
        async def run_test():
            ...
            listener_hub.set_receive_callbacks({OperationType.PLAYER: on_player_received})
    ...
    def test_operation_dict_serialization_roundtrip(self):
        ...
        async def run_test():
            ...
            listener_hub.set_receive_callbacks({OperationType.PLAYER: on_operation_received})
...
class TestCommunicationsIntegration:
    ...
    def test_node_to_controller_via_communications_threads(self):
        ...
        with patch('cuemsengine.comms.ControllerCommunications.Communicator', return_value=mock_comm):
