...
def controller():
    ...
    with patch('cuemsengine.core.BaseEngine.ConfigManager') as MockCM, \
         patch('cuemsengine.core.BaseEngine.BaseEngine.get_controller_ip', return_value='localhost'):
        ...
        mock_cm_instance.library_path = str(Path(__file__).parent / '..' / 'dev' / 'test_xml_files')
...
class TestStopScriptRefactored:
    ...
    def test_stop_calls_clear_playback_state(self, controller):
        ...
        with patch.object(controller, '_clear_playback_state') as mock_clear, \
...
class TestUnloadProject:
    ...
    def test_calls_reset_script(self, controller):
        controller.set_status('running', 'no')
        with patch.object(controller, 'reset_script') as mock_reset, \
             patch.object(controller, '_forward_command_to_nodes'):
            controller.unload_project(None)
...
class TestHandleEditorCommandDictReturn:
    def test_dict_return_passed_as_value(self, controller):
        ...
        with patch.object(controller, 'confirm_to_editor') as mock_confirm, \
    ...
    def test_project_status_in_command_dict(self, controller):
        ...
        with patch.object(controller, 'confirm_to_editor'), \
             patch.object(controller, 'set_editor_request'):
            # Should not raise
