...
class TestResolveControllerHost:
    def test_unicast_address_is_returned(self, env_config_path):
        engine = _bare_engine()
        with patch('cuemsengine.core.BaseEngine.socket.gethostbyname',
                   return_value='169.254.12.139') as gh:
...
class TestGetControllerIpCascade:
    def test_mdns_wins_when_unicast(self, env_config_path):
        ...
        with patch.object(engine, '_resolve_controller_host', return_value='169.254.99.1'):
