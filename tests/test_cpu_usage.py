...
class TestBaseEngineCPUUsage:
    ...
    def test_base_engine_idle_cpu_usage(self, base_engine, engine_cleanup):
        ...
        assert (
            idle_cpu_stats["avg"] < 10.0
        ), f"Idle CPU usage too high: {idle_cpu_stats['avg']}%"
        assert (
            idle_cpu_stats["max"] < 20.0
        ), f"Peak idle CPU usage too high: {idle_cpu_stats['max']}%"
    ...
    @pytest.mark.slow
    @pytest.mark.integration
    def test_base_engine_continuous_operation_cpu_usage(
    ...
    def test_base_engine_long_running_stability(self, base_engine, engine_cleanup):
        ...
        long_term_stats = self.monitor_cpu_usage(
            current_process, duration=15.0, interval=1.0
        )
    ...
    def test_base_engine_cleanup_cpu_usage(self, base_engine, engine_cleanup):
        ...
        assert (
            after_cleanup < 20.0
        ), f"CPU usage after cleanup too high: {after_cleanup}%"
