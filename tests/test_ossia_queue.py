...
def test_global_message_queue_receives_commands(ossia_server_factory, ossia_client_factory):
    ...
    commands_dict = {
        'load': command_callback,
        'gocue': command_callback
    }
    ...
    # ASSERT
    assert len(received_commands) >= 2, f"Expected at least 2 commands, got {len(received_commands)}"
...
def test_global_message_queue_filters_players_by_node_id(ossia_server_factory, ossia_client_factory):
    ...
    with ossia_server_factory(
        ...
        with ossia_client_factory(
            ...
            # Create GlobalMessageQueue and NodeCommunications
            with patch('cuemsengine.comms.NodeCommunications.NodesHub'):
                with patch('cuemsengine.comms.NodeCommunications.PLAYER_HANDLER', mock_player_handler):
                    node_comm = NodeCommunications(
                        hub_address="tcp://127.0.0.1:5555",
                        commands_dict={},
                        node_id=node_id
    ...
    assert len(received_video_messages) >= 1, f"Expected video message, got {len(received_video_messages)}"
    assert len(received_audio_messages) >= 1, f"Expected audio message, got {len(received_audio_messages)}"
    assert len(received_dmx_messages) >= 1, f"Expected DMX message, got {len(received_dmx_messages)}"
    ...
    assert 'test' in video_path and 'path' in video_path, f"Video path incorrect: {video_path}"
    ...
    assert not other_node_video_found, "Other node's video message should be filtered out"


def test_global_message_queue_ignores_unused_paths(ossia_server_factory, ossia_client_factory):
    ...
    received_commands = []
    commands_dict = {
        'load': lambda v: received_commands.append(v)
    }
    ...
    assert len(received_commands) == 1, f"Expected only 1 command, got {len(received_commands)}"
    assert 'test_project' in received_commands, "load command not received"
